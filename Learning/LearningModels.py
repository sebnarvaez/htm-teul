...
#                                     dtype=numpy.uint8)
            ...
            for sentence, actionSeq in trainingData:
                ...
                for word in sentence:
                    ...
                    self.wordSP.compute(encodedWord, True, wordSDR)
                    self.wordTM.compute(
                        set(numpy.where(wordSDR > 0)[0]),
                        True
                    )
                    region1Predicting = region1Predicting or\
                                         (self.wordTM.predictiveCells != set())
                    sentenceActiveCells.update(self.wordTM.getActiveCells())

                if region1Predicting:
                    # Feed the sentence to the region 2
                    #encodedGeneral.fill(0)
                    #encodedGeneral[0][list(sentenceActiveCells)] = 1
                    #self.generalSP.compute(encodedGeneral, True, generalSDR)
                    self.generalTM.compute(sentenceActiveCells, True)
                    #self.generalTM.compute(generalSDR, True)
                ...
                for action in actionSeq:
                    ...
                    self.actionSP.compute(encodedAction, True, actionSDR)
                    self.actionTM.compute(
                        set(numpy.where(actionSDR > 0)[0]),
                        True
                    )
                    actionSeqActiveCells.update(self.actionTM.getActiveCells())

                #if(iteration >= region1PreTraining):
                if region1Predicting:
                    #encodedGeneral.fill(0)
                    #encodedGeneral[1][actionSeqActiveCells] = 1
                    #self.generalSP.compute(encodedGeneral, True, generalSDR)
                    self.generalTM.compute(
                        set([i + nCellsFromSentence for i in actionSeqActiveCells]),
                        True
                    )
                    #self.generalTM.compute(generalSDR, True)
