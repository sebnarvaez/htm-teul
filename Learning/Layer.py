...
#  Fecha última modificación: 2015-11-21
#  Versión: 0.1
...
class Layer():
    ...
    def __init__(self, structure, modules, classifier):
        """
        @param structure: A dictionary containing the module names (keys), and
            their corresponding parent (values).
        @param modules: A dictionary containing the module names (keys) and their
            corresponding object (values). Note that there's no need to include 
            Input modules here, since their values come from the training set.
        @param classifier
    ...
    def applyStructure(self, value, inputName, verbosity=0, recordNum=0, 
        ...
        @param value
        @param inputName: The name of the input module corresponding to the value.
        @param verbosity=0
        @param recordNum=0: The position of the current value in the sequence,
            starting from 0.
        @param learn=True: Whether to enable learning.
        
        @return A dictionary containing:
            'lastModule' : The name of the last module executed (the highest
        ...
        lastOutput = value # This will hold the last output that was generated.
                           # It's uptdated each iteration.
        bucketIdx = 0
        
        if verbosity > 1 : print("Value: " + str(value))
        
        while True:
            
            module = self.modules[moduleName]
            if verbosity > 1 : print("Current module: " + moduleName)
            ...
            if moduleName[-3:] == 'Enc':
                ...
                if verbosity > 1 :
                    ...
                    predictedColumns = module.mapCellsToColumns(module.predictiveCells).keys()
                    print(moduleName + " columns prediction = " + str(predictedColumns))

                lastOutput = module.activeCells
            
            else:
                
                raise ValueError("Invalid Module Name. See the function's docstring "\
                    "to see the correct usage.")
            
            if verbosity > 1 : print(moduleName + " Output = " + str(lastOutput))
    ...
    def processInput(self, inputData, verbosity=0, learn=True):
        ...
        @param inputData: A list of tuples where the first element of each tuple
            is the name of the input module, and the second one is its corresponding
            sequence. Note that the order of the list is important.
        @param verbosity = 0
        @param learn = True: Whether to enable learning.
        """
        ...
        for inputModule in inputData:
            ...
            for value in inputModule[1]:
                structureOutput = self.applyStructure(value, inputModule[0], verbosity,
                    recordNum, learn)
