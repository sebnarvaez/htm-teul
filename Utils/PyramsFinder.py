    ...
    def __repr__(self):
        ...
        return "\nParameter(\n    name='{}',\n    dataType='{}',\n    "\
            ...
            "    mutationProb={}\n)".format(self.name, self.dataType,
                self.value, self.minVal, self.maxVal, self.maxChange,
                self.mutationProb)
...
class Individual:
    ...
    def getParamsDict(self, nonOptimParams):
        """
        @param nonOptimParams
        @return: A dictionary containing the parameter names (keys)
            and values.
        """
        indivParams = {}
...
class ParametersFinder:
    ...
    def _createPopulation(self, populationSize, parents, maxMutations,
            verbosity=0):
        """
    ...
    def findParams(self, populationSize, maxMutations=2, variety=2,
            ...
            randomizeFirstGen=False, maxIterations=200, maxTime=-1,
            minScore=-1, nParallelEvals=1, savingFrequency=-1, savingFunc=None,
            verbosity=0):
        """
        ...
        @param populationSize: The size of the population for each
            generation.
        @param maxMutations: (default=2) The maximum number a
            mutations that is allowed to be done to an individual.
        @param variety: (default=2) The number of individuals that
            will be taken as base for the next generation.
        @param elitism: (default=1) The number of 'elite' individuals
            (i.e. those with the best score so far) that will be
            included in the population for the next generation. If 0
            the new population will consist of freshly generated
            individuals only.
        @param selectionTechnique: (default='RouletteWheel') Can be
            one of the following:
            'RouletteWheel'
            'StochasticUniversalSampling'--> NOT YET IMPLEMENTED
            'Tournament' --> NOT YET IMPLEMENTED
        @param randomizeFirstGen: (default=False) Whether to create
            <variety> individuals with random parameters as parents
            for the first generation, or stay with the individual
            defined by paramsDefinition as the only parent. This can
            provide diversity but may lead the search far away from
            the default values provided in paramsDefinition (if any
            default values were provided).
        @param maxIterations: (default=200) The maximum number of
            Iterations the function will make. Values <= 0 mean no
            maxIterations.
        @param maxTime: (default=-1) The maximum time (in minutes)
            that the algorithm will iterate. The time is checked after
            each iteration, so it won't interrupt any. Values <= 0
            mean no maxTime.
        @param minScore: (default=-1) The minimum score that the
            algorithm must reach until it stops. Note that the
            algorithm may never reach a very high score, so it is
            recommended to always set at least one of the other two
            options (maxIterations and maxTime).
        @param nParallelEvals: (default=1) With this you can evaluate
            multiple individuals from the population at the same time.
            If > 1, the python's multiprocessing module is used. This
            is useful when evalFunc is heavy load. A recommended value
            would be the number of cores of your CPU.
        @param savingFrequency: (default=-1) How often (in
            generations) the best parameters found so far will be
            saved to a file.
        @param savingFunc: (default=None) The function called when its
            time to save. The population (a list of Individuals) and a
            file path are passed as parameters. If None, the params of
            the top 3 individuals will be saved with a default
            formating on {function's-name}_Optim/Iteration_{}.py
        @param verbosity: (default=0) How much verbose about the
            procedure. 0 doesn't print anything.
        ...
        while True:
            ...
            population = self._createPopulation(populationSize, parents, 
            ...
            if (savingFrequency > 0) and \
                    (iterationCount % savingFrequency == 0):
                ...
                if savingFunc is not None:
                    savingFunc(population, self.nonOptimParams, fileName)
                    ...
                    with open(fileName, 'wb') as savingFile:
                        ...
                        savingFile.write("bestFindings = {}\n".format(
                            [individual.getParamsDict(self.nonOptimParams) for \
                                individual in population[:3]]))
            ...
            if ((maxIterations != -1) and (iterationCount >= maxIterations))\
                    or ((maxTime != -1) and (elapsedMinutes >= maxTime)) or\
